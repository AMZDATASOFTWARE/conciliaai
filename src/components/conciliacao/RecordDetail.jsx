...
import { Brain, BookOpen, Check, AlertTriangle } from "lucide-react";

export default function RecordDetail({ record, rule, onClose, onSetStatus }) {
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="bg-slate-900 border-slate-700 text-slate-100 max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">Registro conciliado <StatusBadge status={record.status} /></DialogTitle>
        </DialogHeader>
